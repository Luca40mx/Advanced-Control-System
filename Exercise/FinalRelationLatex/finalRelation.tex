...
\begin{equation}
    \tau = B(q)\ddot{q}_r + C(q, \dot{q})\dot{q}_r + F\dot{q}_r + g(q) + K_D\sigma = Y(q, \dot{q}, \dot{q}_r, \ddot{q}_r)\hat{\Theta} + K_D\sigma
\end{equation}

where:
\begin{itemize}
    \item $\hat{\Theta}$ represents the vector of estimated parameters
    \item $Y(q, \dot{q}, \dot{q}_r, \ddot{q}_r)$ is the regressor matrix that relates the parameters to the dynamics
    \item $\sigma = \dot{\tilde{q}} + \Lambda\tilde{q}$ is a filtered tracking error
    \item $\dot{q}_r = \dot{q}_d - \Lambda\tilde{q}$ and $\ddot{q}_r = \ddot{q}_d - \Lambda\dot{\tilde{q}}$ are reference signals
\end{itemize}
...
\begin{equation}
    \tau = \begin{bmatrix} \ddot{q} & \dot{q} & \sin q \end{bmatrix} \begin{bmatrix} I \\ F \\ G \end{bmatrix}
\end{equation}

where:
\begin{itemize}
    \item $Y(q,\dot{q},\ddot{q}) = \begin{bmatrix} \ddot{q} & \dot{q} & \sin q \end{bmatrix}$ is the regressor matrix
    \item $\Theta = \begin{bmatrix} I \\ F \\ G \end{bmatrix}$ is the parameter vector containing inertia, friction, and gravity terms
\end{itemize}
...
\begin{figure}[H]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.5\textwidth]{../Simulink/Images/AdaptiveControl.png}}
    \caption{Adaptive control scheme}
    \label{fig:adaptiveControl_graph}
\end{figure}
...
\begin{equation}
    M_t\ddot{Z} + K_{Dt}\dot{Z} + K_{Pt}Z = h_e^d
\end{equation}

where

\begin{equation}
    Z = x_d - x_t = -\begin{bmatrix} O_{d,t} \\ \phi_{d,t} \end{bmatrix}
\end{equation}

so by rearranging the terms:

\begin{equation}
    \ddot{x}_t = M_t^{-1}(-h_e^d + K_d\dot{z} + k_p\dot{z} + M_t\ddot{x}_d)
\end{equation}
